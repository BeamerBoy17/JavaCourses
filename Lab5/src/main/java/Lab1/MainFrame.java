...
public class MainFrame extends javax.swing.JFrame {
    ...
    private void CalculateActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_CalculateActionPerformed
        final int threadsCount = Globals.CalculationThreadsCount;
        ...
        while (processedRowsCount != rowIndexes.length){
            int iterationsCount = Integer.min(threadsCount,rowIndexes.length-processedRowsCount);
            try{
                for (int i = 0; i < iterationsCount; i++){
                    double from = Double.parseDouble(
                            jTable1.getValueAt(rowIndexes[i+processedRowsCount],0).toString());
                    double to = Double.parseDouble(
                            jTable1.getValueAt(rowIndexes[i+processedRowsCount],1).toString());
                    double step = Double.parseDouble(
                            jTable1.getValueAt(rowIndexes[i+processedRowsCount],2).toString());
                    threads[i] = new IntegrationThread(from,to,step);
                    threads[i].start();
                }
                for (int i = 0; i < iterationsCount; i++){
                    threads[i].join();
                ...
                JOptionPane.showMessageDialog(null,"Потоки врменно вредничают =(");
                return;
            }
            for (int i = 0; i < iterationsCount; i++){
                dt.setValueAt(threads[i].getResult(), rowIndexes[i+processedRowsCount],3);
            }
